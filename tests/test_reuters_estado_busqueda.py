...
class _PaginaFalsa:
    ...
    def __init__(self, url, html, titulo, texto, url_revienta=False):
        self._url = url
        self._html = html
        self._titulo = titulo
        self._texto = texto
...
def _adaptador(tmp_path, *, url, html, titulo="Reuters Connect", texto="", url_revienta=False):
    ...
    ad._page = _PaginaFalsa(url, html, titulo, texto, url_revienta)
...
def test_sesion_viva_sin_tarjetas_se_lleva_el_diagnostico_puesto(tmp_path):
    """El caso que hoy no sabemos leer: ni sesión muerta ni muro.

    Puede ser cambio de maqueta, cero resultados o la SPA rota, y desde el
    correo no hay manera de distinguirlo. Así que el aviso se lleva la foto de
    la página y nos lo cuenta él la próxima vez.
    """
    ad = _adaptador(
        tmp_path,
        url=URL_BUSQUEDA,
